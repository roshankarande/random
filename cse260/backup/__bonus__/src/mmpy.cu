...
void genMatrix_ISeq(_FTYPE_ *a, _FTYPE_ *b, unsigned int M, unsigned int K, unsigned int N);
void verify(_FTYPE_ *c, _FTYPE_ *a, _FTYPE_ *b, unsigned int m, unsigned int n, _FTYPE_ epsilon, const char *mesg);
void verify(_FTYPE_ *c, _FTYPE_ *a, _FTYPE_ *b, unsigned int m, unsigned int k, unsigned int n, _FTYPE_ epsilon, const char *mesg);
...
void cmdLine(int argc, char *argv[], int &m, int &k, int &n, int &reps, int &ntx, int &nty, _FTYPE_ &eps, int &do_host, int &prefer_l1, int &use_rand, int &use_bt, int &use_seq, int &use_shm_double, int &verify_gpu);
...
void matMulHost(_FTYPE_ *, const _FTYPE_ *, const _FTYPE_ *, unsigned int, unsigned int, unsigned int);
...
int main(int argc, char **argv)
{
    ...
    int m, k, n, reps, ntx, nty, do_host, prefer_l1, use_rand, use_bt, use_seq, use_shm_double, verify_gpu;
    _FTYPE_ eps;

    cmdLine(argc, argv, m, k, n, reps, ntx, nty, eps, do_host, prefer_l1, use_rand, use_bt, use_seq, use_shm_double, verify_gpu);

    // Total amount of storage for entries
    unsigned int mk = m * k * sizeof(_FTYPE_);
    unsigned int kn = k * n * sizeof(_FTYPE_);
    unsigned int mn = m * n * sizeof(_FTYPE_);
    ...
    setGrid(m, n, threads, grid);

    // print configurations
    printf("m: %d, k: %d, n: %d, tx: %d, ty: %d, gridX: %d, gridY: %d, reps: %d, epsilon: %g\n\n", m, k, n, threads.x, threads.y, grid.x, grid.y, reps, eps);
    ...
    _FTYPE_ *h_A = (_FTYPE_ *)malloc(mk);
    assert(h_A);
    _FTYPE_ *h_B = (_FTYPE_ *)malloc(kn);
    assert(h_B);
    if (use_bt)
    {
    ...
    {
        genMatrix_ISeq(h_A, h_B, m, k, n); // I * Seq
    }
    else
    {
        genMatrix(h_A, m, k);
    ...
    if (do_host)
    {
        ...
        hostC = (_FTYPE_ *)malloc(mn);
        t_host = -getTime();
        for (int r = 0; r < reps; r++)
            matMulHost(hostC, h_A, h_B, m, k, n);
        t_host += getTime();
        ...
        if (use_bt)
            ...
            verify(hostC, h_A, h_B, m, k, n, eps, "Host result");
    ...
    cudaFuncSetCacheConfig(kernel, Preference);
...
// execute the kernel
    ...
    for (int r = 0; r < SCALE * reps; r++)
#ifdef TARGET_T4
        // cout << kernel_description;
        kernel<<<grid, threads, 64 * 1024>>>(m, k, n, d_C, d_A, d_B);
#endif
#endif
    ...
    cudaMemcpy(h_C, d_C, mn, cudaMemcpyDeviceToHost);
    ...
    if (verify_gpu)
    {
        ...
        //     verify_ISeq(h_C, n, n);
            ...
            verify(h_C, h_A, h_B, m, k, n, eps, "Device result");
    }
    ...
    if (do_host)
        ...
        if (use_bt)
            ...
            verify(h_C, hostC, m, n, eps, "Device vs. host");
